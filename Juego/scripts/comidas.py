...
class Comidas():
    ...
    def determinar_posicion_comida(self, posicion_bliteo:str,pantalla,lista_pedidos:list = []):
        match posicion_bliteo:
            case "barra":
                if self.numero_pedido < 7:
                    self.lugar = "barra"
                    self.rect_comida.x = BARRA_Y
                    self.rect_comida.y = (ESPACIO_ARRIBA + MEDIDAS_LOSA[1] * (len(lista_pedidos) - 1))
            case "mesa":
                self.lugar = "mesa"
