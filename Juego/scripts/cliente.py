...
class Cliente():
    ...
    def revisar_paciencia_cliente(self, tiempo_juego:int):
        ...
        if (tiempo_juego - self.tiempo_espera) > TIEMPO_ANTES_ENOJO:
            self.imagen_bliteo = self.imagen_enojado
            self.animo = "ira"
        elif (tiempo_juego - self.tiempo_espera) > TIEMPO_ANTES_NEUTRAL:
            self.animo = "enojo"
        elif (tiempo_juego - self.tiempo_espera) > TIEMPO_ANTES_FELICES:
            self.animo = "neutral"
        else:
            self.animo = "feliz"
